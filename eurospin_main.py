...
LINK_COLLECTION_DIR = Path(__file__).resolve().parent / "link_collection" / "eurospin"
RAW_DATA_DIR = Path(__file__).resolve().parent / "raw_data" / "eurospin"
...
async def main():
    ...
    if 1 in stages:
        await collect_links(EUROSPIN, max_pages=args.pages)
    ...
    if 2 in stages:
        ...
        cfg = dataclasses.replace(EUROSPIN, **overrides) if overrides else EUROSPIN
        await scrape_raw(
            cfg,
            links_folder,
